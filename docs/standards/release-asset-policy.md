...
- `ffi/sysprims-ffi/sysprims.h` is generated by `cbindgen` for local builds and for release packaging; it is gitignored.
- `bindings/go/sysprims/include/sysprims.h` is committed because it is part of the Go binding source surface.
- The header shipped as a release asset (typically `sysprims.h` at the release top-level) is authenticated via signed checksums.
