...
**Release Date:** TBD (planning)
**Status:** Draft planning scope (pre-implementation)

> This document is intentionally forward-looking and captures proposed scope.
> It is not a statement of implemented behavior until each brief lands and is
...
pub struct ProcessInfo {
    // ... existing fields ...
    pub env: Option<HashMap<String, String>>,  // proc_ext
...
```rust
use sysprims_proc::{get_process, snapshot, ProcessOptions};

// Get a single process with env and thread count
let proc = get_process(pid, ProcessOptions::with_env().with_threads())?;
...
let snap = snapshot(ProcessOptions::with_threads())?;
```
...
proc, err := sysprims.GetProcess(pid, sysprims.WithEnv(), sysprims.WithThreads())
if err != nil {
    return err
}
...
procs, err := sysprims.ProcessList(
    sysprims.WithFilter(&sysprims.ProcessFilter{NameContains: "worker"}),
    sysprims.WithThreads(),
)
```
...
**FFI** (`config_json` gains two optional boolean fields):

```c
// config_json: {"pid": 1234, "include_env": true, "include_threads": true}
sysprims_proc_get(config_json, &result_json_out);
```
...
```rust
use sysprims_proc::{descendants, DescendantsConfig, CpuMode};
...
let config = DescendantsConfig {
    ...
    filter: ProcessFilter::builder().cpu_above(80.0).build(),
    ..Default::default()
};
let result = descendants(config)?;
```

**Go binding**:

```go
// The complete surgical cleanup workflow — impossible in v0.1.13
...
**Schema**: `DescendantsConfig` input schema gains two optional fields (minor). Output schema
(`DescendantsResult`) unchanged.

---

#### Contextual hint: `--cpu-above` without `--cpu-mode monitor`

When `--cpu-above` is specified and `--cpu-mode` is the default (`lifetime`) on `descendants`,
`kill-descendants`, or `kill`, emit one line to stderr:
...
*These are handed to prodmktg / developer relations at feature-complete, before release.*
...
|--------------------|---------------------|
| `exec.Command("ps", "eww", "-p", pid)` + text parse | `sysprims.GetProcess(pid, WithEnv())` |
| `exec.Command("ps", "-M", "-p", pid)` + count lines | `sysprims.GetProcess(pid, WithThreads())` |
...
Update to add `GetProcess()` with `WithEnv()` and `WithThreads()`, and the
`KillDescendants()` with `CpuMode` workflow. The README is the first thing a Go engineer
...
- The kill gate (`--yes` / `Yes: true`) remains unchanged.
- `--dry-run` / `DryRun: true` always previews before acting regardless of cpu-mode.
...
| `descendants-config.schema.json` | v1.0.0 | v1.1.0 | Add `cpu_mode`, `sample_duration_ms` (optional) |
| `descendants-result.schema.json` | v1.0.0 | v1.0.0 | No change |
| `kill-descendants-config.schema.json` | v1.0.0 | v1.1.0 | Add `cpu_mode`, `sample_duration_ms` (optional) |
