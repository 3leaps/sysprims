...
function main() {
  ...
  const alreadyVendored = fs
    ...
    .some((d) => {
      ...
      try {
        return fs.readdirSync(dir).some((f) => f.endsWith('.so') || f.endsWith('.dylib') || f.endsWith('.dll'));
      } catch {
        return false;
      }
    });
  ...
  if (!fs.existsSync(localRelease)) {
    if (alreadyVendored) {
      console.log(`[ok] _lib already populated; skipping local vendoring (missing ${localRelease})`);
      return;
    }
    console.error(`Missing local sysprims-ffi staging dir: ${localRelease}`);
    console.error('Either populate _lib/<platform>/ with shared libs, or run: make build-local-ffi-shared');
    process.exit(1);
  }
