...
type SysprimsLib = {
  ...
  sysprims_free_string: (ptr: any) => void;

  sysprims_proc_get: (pid: number, out: any[]) => number;
  sysprims_self_getpgid: (out: any[]) => number;
  sysprims_self_getsid: (out: any[]) => number;
};
...
function raiseSysprimsError(lib: SysprimsLib, code: number): never {
  ...
  throw new SysprimsError(code as any, msg && msg.length > 0 ? msg : `sysprims error${codeNameSuffix}`);
}
...
export function loadSysprims(): SysprimsLib {
  ...
  const SysprimsOwnedStr = koffi.disposable('SysprimsOwnedStr', 'str', sysprims_free_string);
  ...
  const api: SysprimsLib = {
    ...
    sysprims_last_error_code: lib.func('int32_t sysprims_last_error_code(void)'),
...
export function callU32Out(fn: (outPtr: any[]) => number, lib: SysprimsLib): number {
  lib.sysprims_clear_error();
  const out: any[] = [0];
  const code = fn(out);
  if (code !== SysprimsErrorCode.Ok) {
